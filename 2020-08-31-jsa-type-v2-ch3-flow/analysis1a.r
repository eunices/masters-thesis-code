...
# des_where_summary_plot$Class = des_where_by_class[
#     match(des_where_summary_plot$Class, des_where_by_class$Class),
# ]$name
#
# des_where_summary_plot$variable = des_where_by_continent[
#     match(des_where_summary_plot$variable, rownames(des_where_by_continent)),
# ]$name

labels_x <- 
    des_where_by_class[match(lvls_class, des_where_by_class$Class),]$name

labels_y <- 
    des_where_by_continent[
        match(lvls_continent, rownames(des_where_by_continent)),
    ]$name
...
length(unique(des_where2[Class == "Unclassed"]$residence.country.describer)) # N unclassed
...
ggsave(
    paste0(dir_plot, 'fig-3.png'), p1, units="cm",
...
write.csv(
    ss_summary,
    paste0(v2_dir_data_ch3_flow, "2019-09-22-summary-country-prop-summary.csv"),
    na='', row.names=F, fileEncoding="UTF-8"
)

write.csv(
    ss[order(-prop)],
    paste0(v2_dir_data_ch3_flow, "2019-09-22-summary-country-prop.csv"), 
    na='', row.names=F, fileEncoding="UTF-8"
)
...
# Data processing
flow <- unique(spp_s[, c("type.country_n", "residence.country.describer.first")])
...
ggsave(
    paste0(dir_plot, 'fig-4.png'), p2, units="cm",
    width=20, height=10, dpi=300
)
...
write.csv(
    ...
    paste0(v2_dir_data_ch3_flow, "2019-09-22-summary-country-N-summary.csv"), 
    na='', row.names=F, fileEncoding="UTF-8"
)
