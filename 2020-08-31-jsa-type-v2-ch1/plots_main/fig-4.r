...
rng <- range(species_and_pub_per_year$date.n)
species_and_pub_per_year <- 
    data.table(merge(data.frame(date.n=seq(rng[1], rng[2])), 
                     species_and_pub_per_year,
...
p16 <- ggplot(data=taxonomic_effort, aes(x=years, y=species_per_real_taxonomist_weighted)) +
    ...
    geom_line(size=.5, color='grey', linetype='dashed') +
    geom_smooth(fill=NA, color='black', size=1.5) +
    geom_line(size=1, aes(y=species_per_real_taxonomist_weighted_roll), color='grey50') +
    scale_x_continuous(breaks=ybreaks50, minor_breaks=ybreaks10) +
    scale_y_continuous(breaks=ybreaks2, minor_breaks=ybreaks1, limits=c(0,12))
...
ts_data_sp_per_pub <- ts(species_and_pub_per_year$species_per_publication,
                         frequency = 1, start = c(1758))
ts_data_species_per_tax <- ts(taxonomic_effort$species_per_real_taxonomist,
                              frequency = 1, start = c(1758))
...
idx_1910 <- which(species_and_pub_per_year$date.n == 1907)
...
summary(res)
res <- MannKendall(ts_data_species_per_tax[idx_1910:length(ts_data_species_per_tax)])
summary(res)
...
taxonomic_effort$roll <- rollmean(taxonomic_effort$species_per_real_taxonomist, 10, 
                                  fill = list(NA, NULL, NA))
plot(taxonomic_effort$years, taxonomic_effort$ma)
mod <- lm(species_per_real_taxonomist~years, data = taxonomic_effort)
seg <- segmented(mod, seg.Z = ~ years, psi = list(years=1910))

mod <- lm(ma~years, data = taxonomic_effort[!is.na(ma)])
seg <- segmented(mod, seg.Z = ~ years, psi = list(years=1910))
