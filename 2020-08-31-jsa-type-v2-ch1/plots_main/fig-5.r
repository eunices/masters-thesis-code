...
des_y <- merge(des_y, des_y_roll, by=c("years", "variable"), suffixes=c("", "_roll"))
...
for (i in 1:10) {
  ...
  print(paste0("ANALYSING FOR ", name))
  trend <- te_trend[, get(name)]
  plot(trend)
  res <- MannKendall(trend)
  # summary(res)
  # print(res$tau>0)
  print(round(as.numeric(res$tau), 2))
  print(round(as.numeric(res$sl), 2))
...
res <- MannKendall(taxonomic_effort$N_real_describers.6.prop_roll)
summary(res)
res <- MannKendall(taxonomic_effort$N_real_describers.7.prop_roll)
summary(res)
...
formatstr <- function(string) {
  ...
  string <- ifelse(string=="1", paste0(string, " species"), paste0("<=", string, " species"))
}

des_y$variable <- factor(des_y$variable, levels=paste0("N_real_describers.", 1:10, ".prop"))
