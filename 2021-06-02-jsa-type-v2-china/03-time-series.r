...
# Check
df[type.country_n=="CH" & status =="Valid species", .N]

n_species <- df[type.country_n=="CH" & status =="Valid species", .N, by="date"]
n_species$date <- as.integer(n_species$date)
n_species <- n_species[order(date)]
n_species$N <- cumsum(n_species$N)
...
n_species <- df[type.country_n=="CH" & status =="Valid species",]
...
x <- n_species[,.N, by=NAME_1][N>10]
n_species <- n_species[NAME_1 %in% x$NAME_1,.N, by=c("date", "NAME_1")]

n_species$date <- as.integer(n_species$date)
n_species <- n_species[order(NAME_1, date)]

template <- expand.grid(date=seq(min(n_species$date), max(n_species$date)), NAME_1=x$NAME_1)
n_species <- merge(template, n_species, by=c("NAME_1", "date"), all.x=T, all.y=F)
n_species <- data.table(n_species)
n_species[is.na(N)]$N <- 0 
n_species[, N := cumsum(N), by="NAME_1"]
...
ggsave(paste0(v2_dir_china, '03-time-series-01b-new.png'), p, units="cm", width=15, height=18, dpi=300)
