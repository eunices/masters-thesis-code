...
df_dist <- df_dist[, list(
        country = paste0(sort(unique(global.mapper_n)), 
            collapse = "; "),
        continent = paste0(sort(unique(continents.mapper_n)), 
            collapse = "; "),
        biogeo_wwf = paste0(sort(unique(biogeo_wwf.mapper_n)),
             collapse = "; "),
        biogeo_ecor2017 = paste0(sort(unique(biogeo_ecor2017.mapper_n)),
             collapse = "; "),
        latitude = paste0(sort(unique(latitude_type2.mapper_n)),
             collapse = "; ")
    ), 
    by = cols
]
