...
print(paste0(Sys.time(), " ----- clean06.r"))
...
# Count synonyms
sum <- df[
    duplicated == FALSE & status %in% c("Synonym"),
    list(N = .N), by=c("corrected_valid_species", "status")
]

var_count <- dcast(
    sum, corrected_valid_species ~ status, value.var = "N"
)
...
subsp_count <- subsp_count[
    !(genus == "" | species == "" | is.na(genus) | is.na(species))
]
