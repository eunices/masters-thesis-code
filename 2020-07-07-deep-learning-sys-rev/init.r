...
data_dir_rev <- paste0(data_dir, "review-v1/")
data_dir_wos_raw <- paste0(data_dir_rev, "wos-raw/")
data_dir_sco_raw <- paste0(data_dir_rev, "scopus-raw/")
data_dir_wos_cle <- paste0(data_dir_rev, "wos-clean/")
data_dir_sco_cle <- paste0(data_dir_rev, "scopus-clean/")
data_dir_lp <- paste0(data_dir, "lookup/")

# Lookup files
lp_file_cols <- paste0(pub_dir_table, "2020-07-08-dl-studies.xlsx")
lp_cols <- names(read.xlsx2(lp_file_cols, sheetName = "all"))[1:21]
